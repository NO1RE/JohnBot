 ...
 */
package net.liveware.discord.util;

import net.liveware.discord.Permissions;
import net.liveware.discord.commands.TodoCommand;
import net.liveware.discord.commands.TouhouCommand;
...
public class Database
{
    ...
    private Database()
    {
        ...
        try
        {
            ...
            conn = DriverManager.getConnection("jdbc:sqlite:discord.db");
            ...
            //2hu
            preparedStatements.put(Permissions.ADD_OP, conn.prepareStatement("REPLACE INTO TouhouDatabase (id) VALUES (?)"));
            preparedStatements.put(Permissions.GET_OPS, conn.prepareStatement("SELECT id FROM TouhouDatabase"));
            preparedStatements.put(Permissions.REMOVE_OPS, conn.prepareStatement("DELETE FROM TouhouDatabase WHERE id = ?"));
            ...
            //2hu
            preparedStatements.put(TouhouCommand.ADD_TOUHOU_CHARACTER, conn.prepareStatement("INSERT INTO TouhouDatabase (name, summary, image) VALUES (?, ?, ?)"));
            preparedStatements.put(TouhouCommand.GET_TOUHOU_CHARACTER, conn.prepareStatement("SELECT id, name, summary, image FROM TouhouDatabase"));
            preparedStatements.put(TouhouCommand.SET_TOUHOU_CHARACTER, conn.prepareStatement("UPDATE TouhouDatabase SET id = ?, name = ?, summary = ?, image = ? WHERE id = ?"));
            preparedStatements.put(TouhouCommand.REMOVE_TOUHOU_CHARACTER, conn.prepareStatement("DELETE FROM TouhouDatabase WHERE id = ?"));
        }
